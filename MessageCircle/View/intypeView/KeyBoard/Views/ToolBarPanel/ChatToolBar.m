...
- (void)initSubviews
{
    ...
    self.sentBtn = [self createBtn:kButKindSent action:@selector(toolbarBtnClick:)];
...
- (void)setbarSubViewsFrame
{
    ...
    if (self.allowSentButton){
        self.sentBtn.frame = CGRectMake(self.frame.size.width - ItemW - CGRectGetWidth(self.sentBtn.frame), barViewH - ItemH, ItemW, ItemH);
    }else {
        self.sentBtn.frame = CGRectZero;
    }
...
- (void)prepareForBeginComment
{
    ...
    self.sendSelected = self.sentBtn.selected = NO;
...
selectStateImageStr:(NSString *)selectStr highLightStateImageStr:(NSString *)highLightStr
{
    ...
    UIImage *normal = [UIImage imageNamed:@"face"];
    UIImage *select = [UIImage imageNamed:@"keyboard"];
//    UIImage *highlight = [UIImage imageNamed:@"face_HL"];
  //    [btn setTitle:NSLocalizedString(@"表情", "") forState:UIControlStateNormal];
...
- (void)toolbarBtnClick:(UIButton *)sender
{
    switch (sender.tag) {
        ...
        case 5:
        {
            [self handelSentBarClick:sender];
            break;
        }
        default:
            break;
...
- (void)handelSentBarClick:(UIButton *)sender{
...
- (void)setAllowSendButton:(BOOL)allowSentButton
{
    _allowSentButton= allowSentButton;
    
    if (_allowSentButton) {
        self.sentBtn.hidden = NO;
    }else {
        self.sentBtn.hidden = YES;
    }
