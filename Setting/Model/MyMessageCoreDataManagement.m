...
#import "MyMessageCoreDataManagement.h"
#import "Account.h"

@implementation MyMessageCoreDataManagement
      ...
      commentDataArray:(NSMutableArray *)commentsDataArray
{
    ...
    MyMessageCircle_main *message = [NSEntityDescription insertNewObjectForEntityForName:TableName inManagedObjectContext:context];
    ...
    for(NSMutableDictionary *commentInfo in commentsDataArray){
        MyMessageCircle_comments *a = [NSEntityDescription insertNewObjectForEntityForName:relationshipTableName inManagedObjectContext:context];
...
- (NSMutableArray*)selectData:(int)pageSize andOffset:(int)currentPage
{
    ...
    for (MyMessageCircle_main *info in fetchedObjects) {
...
- (void)updateLike:(NSString*)someThing  withString:(NSString*)string
{
    ...
    for (MyMessageCircle_main *info in result) {
...
- (void)updateComment:(NSString*)someThing  withComment:(NSMutableDictionary *)commentInfo
{
    ...
    NSError *error = nil;
    NSArray *result = [context executeFetchRequest:request error:&error];//这里获取到的是一个数组，你需要取出你要更新的那个obj
    MyMessageCircle_main *info = result[0];
    
    
    MyMessageCircle_comments *a = [NSEntityDescription insertNewObjectForEntityForName:relationshipTableName inManagedObjectContext:context];
    a.commentID = [commentInfo objectForKey:@"commentID"];
