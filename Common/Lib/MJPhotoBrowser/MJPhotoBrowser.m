...
{
    if (!error) {
        Alert * alert = [[Alert alloc]initWithTitle:NSLocalizedString(@"保存图片", "") message:NSLocalizedString(@"成功","") delegate:nil cancelButtonTitle:NSLocalizedString(@"好", "") otherButtonTitles: nil];
        [alert show];
    }else
    {
    ...
    if(gesture.state == UIGestureRecognizerStateBegan)
    {
        UIActionSheet *actionSheet = [[UIActionSheet alloc]
                                     initWithTitle:nil
                                      ...
                                      otherButtonTitles:NSLocalizedString(@"保存图片", ""),nil];
        actionSheet.actionSheetStyle = UIActionSheetStyleBlackOpaque;
        [actionSheet showInView:self.view];
        _sentImg = (MJPhotoView*)[gesture view];
...

{
    ...
    if (buttonIndex == 0) {
        ...
        UIImageWriteToSavedPhotosAlbum(_sentImg.photo.image, self, @selector(imageSavedToPhotosAlbum:didFinishSavingWithError:contextInfo:), nil);
